...
async function setSelectPanorama(hotSpot) {
    ...
    if (photoValue) {
        ...
        try {
            const jsonData = await loadHotSpotsFromJson(jsonUrl);
            ...
            await finalizeScene(imageUrl, isExternal, hotspotsFromJson, cameraDirection);
        } catch(error) {
            console.log("No JSON file found or error loading:", jsonUrl, error);
            // Загружаем сцену без данных из JSON
            await finalizeScene(imageUrl, isExternal, [], {});
        }
    }
    else if (infoValue) {

...
function onImageLoad() {
    ...
    if (!listenersAdded) {
        ...
        dragFix.addEventListener('contextmenu', function(event) {
            ...
            // Проверяем, кликнули ли мы по HotSpot
            var clickedHotSpot = findHotSpotAtCoords(coords[1], coords[0]);
            if (clickedHotSpot && config.onContextMenuHotSpot) {
                // Вызываем обработчик для HotSpot
                ...
                // Вызываем обработчик для сцены (если не попали в HotSpot)
...
/**
 * Ищет HotSpot по координатам (yaw, pitch)
 * @private
 * @param {number} yaw - Координата yaw
 * @param {number} pitch - Координата pitch
 * @returns {Object|null} Найденный HotSpot или null
 */
function findHotSpotAtCoords(yaw, pitch) {
    if (!config.hotSpots || !loaded) {
        return null;
    }
    ...
    // Проверяем каждый HotSpot
    for (var i = 0; i < config.hotSpots.length; i++) {
        var hs = config.hotSpots[i];

        // Проверяем, находится ли точка в пределах HotSpot
        // Используем допуск в 5 градусов для определения попадания
        var tolerance = 5;
        var hsYaw = hs.yaw || 0;
        var hsPitch = hs.pitch || 0;

        // Нормализуем yaw HotSpot
        hsYaw = ((hsYaw + 180) % 360) - 180;

        // Вычисляем угловое расстояние
        var yawDiff = Math.abs(yaw - hsYaw);
        var pitchDiff = Math.abs(pitch - hsPitch);

        // Учитываем циклическую природу yaw
        if (yawDiff > 180) {
            yawDiff = 360 - yawDiff;
        }

        if (yawDiff <= tolerance && pitchDiff <= tolerance) {
            return hs;
        }
    }
...
function onDocumentMouseDown(event) {
    ...
    // Проверяем, правая ли это кнопка мыши (button = 2)
    if (event.button === 2) {
        // Получаем координаты в сцене
        var coords = mouseEventToCoords(event);
        var pos = mousePosition(event);
 ...
 */
    function renderHotSpot(hs) {
        ...
        if ((hs.yaw <= 90 && hs.yaw > -90 && z <= 0) ||
            ...
            // Option 4: Use custom scale from hotSpot configuration if provided
            if (hs.customScale !== undefined) {
                scale = hs.customScale;
            }
